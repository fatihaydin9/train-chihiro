...
export class ProceduralSounds implements Updatable {
  ...
  constructor(
    ...
    this.wheelGain.gain.value = 0.07;
    ...
    this.eventBus.on('biome:transition-tick', (config) => {
      ...
      if (wt === 'rain' || wt === 'storm' || wt === 'drizzle' || wt === 'hail') {
        this.rainTargetVol = intensity * 0.10;
      } else {
  ...
  update(dt: number): void {
    ...
    this.rumbleGain.gain.value = 0.14 + s * 0.06;
    this.rumbleFilter.frequency.value = 55 + s * 25;

    // Wheel hum: higher center freq + louder when fast
    this.wheelGain.gain.value = 0.07 + s * 0.04;
    this.wheelFilter.frequency.value = 140 + s * 80;

    // Rail hiss: louder when fast (wind/speed creates more high-freq)
    this.hissGain.gain.value = 0.02 + s * 0.03;
    this.hissFilter.frequency.value = 2500 - s * 500; // let more through

    // Clack interval: shorter when fast
    this.clackInterval = 0.75 - s * 0.25;
    ...
    if (this.clackTimer >= this.clackInterval) {
      ...
      const vol1 = 0.14 + s * 0.04;
      const vol2 = 0.08 + s * 0.03;
      // First hit
    ...
    if (this.rainTargetVol <= 0 && rainCur < 0.002) {
      ...
      this.rainGain.gain.value = rainCur + (this.rainTargetVol - rainCur) * Math.min(dt * 2, 1);
    }

    // Drip modulation
    if (this.rainGain.gain.value > 0.01) {
      this.rainDripTimer += dt;
      const dripMod = 0.85 + 0.15 * Math.sin(this.rainDripTimer * 12.0)
    ...
    if (this.windTargetVol <= 0 && windCur < 0.002) {
      ...
      this.windGain.gain.value = windCur + (this.windTargetVol - windCur) * Math.min(dt * 2, 1);
    }
  }
}
